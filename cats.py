...
def about(topic):
    ...
    def selector(paragraph):
        cleaned_words = lower(remove_punctuation(paragraph)).split(" ")
        for elements in cleaned_words:
            if elements in topic:
                return True
        return False
    return selector
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if len(typed_words)== 0 and len(reference_words) == 0:
        ...
        for count in range(len(typed_words)):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for count in range(len(word_list)-1, -1, -1):
        if typed_word ==  word_list[count]:
            return word_list[count]
        elif diff_function(typed_word, word_list[count], limit) <= difference:
            difference = diff_function(typed_word, word_list[count], limit)
            returned_word = word_list[count]
...
def sphinx_swaps(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(new_start, new_goal, count):
        if start == goal:
            return 0
        elif count > limit:
            return limit+1
        elif not new_start and not new_goal:
            return count
        elif not new_start or not new_goal:
            return helper(new_start[1:], new_goal[1:], count + 1)
        elif new_start[0] == new_goal[0]:
            return helper(new_start[1:], new_goal[1:], count)
        else:
            return helper(new_start[1:], new_goal[1:], count + 1)
...
def time_per_word(words, times_per_player):
    ...
    for elem in times_per_player:
        new_times.append([elem[i+1] - elem[i] for i in range(0, len(elem)-1)])
...
def fastest_words(match):
    ...
    for x in word_indices:
        ...
        for y in player_indices:
            just_times.append(time(match, y, x))
        lowest_time =  min(just_times)
        fastest_player = just_times.index(lowest_time)
        final_list[fastest_player].append(word_at(match, x))
    return final_list
